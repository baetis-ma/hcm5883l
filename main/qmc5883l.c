...
#define I2C_MASTER_FREQ_HZ   100000 // max frequency of i2c clk
...
void adc_task () {
    ...
    uint8_t regdata[256];
    while(1){
        i2c_read(I2C_MASTER_NUM, 0x00, regdata, 6); 
...
void app_main()
{
    ...
    uint8_t data[8];
    data[0] = 0x01;
    i2c_write(I2C_MASTER_NUM, 0x09, data, 1); cycles(1);
    data[0] = 0x00;
    i2c_write(I2C_MASTER_NUM, 0x0a, data, 1); cycles(1);
    data[0] = 0x01;
    i2c_write(I2C_MASTER_NUM, 0x0b, data, 1); cycles(1);

    i2c_read (I2C_MASTER_NUM, 0x0b, data, 1);
